...
def make_pie(slider_input, title):
    fig = go.Figure(
        data=[
            go.Bar(
                x=["Cash", "Bonds", "Stocks"],  # Categories (same as pie chart labels)
                y=slider_input,  # Values for each category
                text=slider_input,  # Show the values on top of the bars
                textposition="auto",  # Position text inside the bars
                marker={"color": [COLORS["cash"], COLORS["bonds"], COLORS["stocks"]]},  # Custom colors
                hoverinfo="none", # Display hover info

            )
        ]
    )
    fig.update_layout(
        title_text=title + "  (Total = 100%)",
        title_x=0.5,
...
slider_card = dbc.Card(
    [
        ...
        html.Div(id="bonds_allocation",className="card-title mt-4", style={"fontSize": "15px", "fontWeight":"bold"})
    ],
    body=True,
    className="mt-4",
)
...
#previous_button = dbc.Button("Previous settings", id = "previous_button", color = "primary", disabled=True)
...
def backtest(stocks, cash, start_bal, nper, start_yr):
    ...
    for yr in dff.Year + 1:
        if yr <= end_yr:
            ...
            dff.loc[yr, "Total"] = dff.loc[yr, ["Cash", "Bonds", "Stocks"]].astype(int).sum()
...
app.layout = dbc.Container(
    [
        # dbc.Row(
        #     dbc.Col(
        #         dcc.Store(id = "stored_history", data=[]),
        #         dcc.Store(id="current_settings", data = {"param1": start_amount, "param2": start_year, "param3": number_of_years})
        #     )
        # ),
        dbc.Row(
            dbc.Col(
...
"""
@app.callback(
    Output("bonds_allocation", "children"),
    Input("stock_bond", "value"),
    Input("cash", "value"),
)
def update_bonds(stocks,cash):
    bonds = 100 - stocks - cash
    return f"Bonds allocation: {bonds}%"
